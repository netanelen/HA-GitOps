# (Imports and client setup from your file are fine)
# ...
@app.route("/")
def home():
    # Fetch EC2 instances
    ...
    # Fetch VPCs
    vpcs = ec2_client.describe_vpcs()  # <--- BUG FIX: Actually call the API
    vpc_data = [{"VPC ID": vpc["VpcId"], "CIDR": vpc["CidrBlock"]} for vpc in vpcs["Vpcs"]]

    # Fetch Load Balancers
    lbs = elb_client.describe_load_balancers()  # <--- BUG FIX: Actually call the API
    lb_data = [{"LB Name": lb["LoadBalancerName"], "DNS Name": lb["DNSName"]} for lb in lbs["LoadBalancers"]]

    # Fetch AMIs (owned by the account)
    amis = ec2_client.describe_images(Owners=['self'])  # <--- BUG FIX: Actually call the API
